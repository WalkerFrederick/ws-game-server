...
const MAX_ROUNDS = 4; // Maximum number of rounds in a game
const WINNING_SCORE = 2; // Score required to win the game
const COUNTDOWN_SECONDS = 5; // Countdown time before the game starts
const RECONNECT_TIME_SECONDS = 60; // Allowed time for a player to reconnect
const ROUND_BUFFER_SECONDS = 3; // Buffer time before starting a new round after one ends
    ...
    socket.on("disconnect", () => {
      Object.keys(games).forEach((gameId) => {
        ...
        if (playerIndex !== -1) {
          ...
          const remainingPlayer = game.players.find((p) => p.id !== socket.id);
          if (remainingPlayer) {
            io.to(remainingPlayer.id).emit("server:notification", {
              type: "error",
...
function startRound(io: Server, gameId: string) {
  ...
  const countdownInterval = setInterval(() => {
    ...
    if (countdown > 0 && !(game.players[0].choice && game.players[1].choice)) {
      io.to(gameId).emit("server:round-countdown", { countdown });
    } else {
      clearInterval(countdownInterval);
...
function checkRoundEnd(io: Server, gameId: string) {
  ...
  io.to(gameId).emit("server:round-result", {
    player1: { id: player1.id, choice: player1.choice, score: player1.score, wonRound: winner === 1 },
    player2: { id: player2.id, choice: player2.choice, score: player2.score, wonRound: winner === 2 },
  });

  // Emit round-end-timer countdown every second until the next round starts
  let countdown = ROUND_BUFFER_SECONDS;
